...
In the previous guide, we created our initial cohort spine, whether using an NPD or HES inception. We will now extract demographic and SEN data. We are doing this across three different scripts because we are taking data at different time points/using different summaries (Table 1). Our choices here, including time frame, are more or less arbitrary and are chosen for the purposes of illustrating different ways of summarising and extracting this kind of data.

| Variable | Nature | Time point
| Gender | Time invariant (a) | Modal value across reception to year 7 |
| Ethnicity | Time invariant (a) | Modal value across reception to year 7 |
| First language group major | Time invariant (a) | Modal value across reception to year 7 |
...
: Table 1. Demographic data to be extracted. FSM free school meals; IDACI income deprivation affecting children index; SEN special educational needs. * This is not necessarily the case. See text that follows.
...
| Variable | Cleaning |
| Gender | Sets values `0` and `9` to `NA.` Then creates a variable called female, where `1` indicates female, and `0` male. This is a practice for binary variables that we recommend generally as there can never be confusion as to the coding. Binary variables coded as `1` and `2` should certainly be avoided. Then we drop the now-redundant gender variable. |
| Ethnicity | Ethnicity	Ethnic groups are combined into the five major ethnic groups. |
| Language | Language	Languages are combined into English or not English. |
...
In order to get the modal value, we use `mode_fun()`, which finds the modal element of a vector or, where the vector we supply is multimodal (i.e., there are at least two options that occur equally with the highest frequency), we pick one at random using a single random draw from a multinomial distribution (Code Snippet 5, from [00b_prelim.r](scripts/00b_prelim.r)).

*Code Snippet 5 (from [00b_prelim.r](scripts/00b_prelim.r))*
``` r
---
mode_fun <- function(x) {
  v <- x[!is.na(x)]
...
We use `mode_fun()` (Code Snippet 5) to get the modal value of each of the time invariant demographic variables by PMR, i.e., using all of each child’s records from reception to year 7 (Code Snippet 7). We first set a seed because we use a stochastic process (random draw from a multinomial distribution) in `mode_fun()`. 
...
# Values at year 7
Script [03_npd_demographic_year7.r](scripts/03_npd_demographic_year7.r) is significantly simpler, as we are only dealing with one time point. Again, we extract data from the censuses, alternative provision and pupil referral units and deduplicate. We use `mode_fun()` again that we defined in the previous script. We then also save our resulting object, which contains PMR, FSM and IDACI data at year 7.


# “Ever” values across reception to year 7

Finally, script [04_npd_demographic_ever.r](scripts/04_npd_demographic_ever.r) is a little more complex. We first extract data. We then identify which children are enrolled in special school schools (which we flag using a variable called `specialist_school`). To do this, we use the Get Information About Schools database and link the relevant information using the school unique reference number (URN) (Code Snippet 8). [Note: this step is still under construction.]
...
Next, we clean our SEND variable to combine the old Action and Action Plus values with the new Support value. “Support” replaced Action/Action Plus from academic year 2014/15. Similarly, our SEN variable combines the old statements of SEN with the new Education, Health & Care Plan (which were phased in from 2014/15). We also add children enrolled in alternative provision or pupil referrals units to our `specialist_school` flag, as children only in alternative provision will not be captured in the Get Information About Schools database (Code Snippet 9).
...
Next, we define `fsm_ever_primary` by taking the maximum value of `fsmeligible` across all records (Code Snippet 10). If there is ever a `1` (the maximum of `0` and `1`), then this operation will return a `1` by child.
...
Finally, we retain just four variables, PMR, `fsm_ever_primary`, senprovision and `specialist_school.` Note that we so far have not defined an “ever SEN” variable. We will do this in the next guide. For now, we just save the resulting object as its own file and clear the workspace.

# Coming up...

In the [next Guide](guide05_demo_hes.qmd), we will think about why we might now also want to extract demographic data from HES and how you can adapt code provided in this Guide to do so.
