...
*Code Snippet 8 (R - 01a_identify_hes_cohort.r)*
``` r
---
...
The rest of the script is relatively straightforward. The next section identifies birth episodes. Here we make use of the code list for identifying births that is available through the [ECHILD Phenotype Code List Repository](https://code.echild.ac.uk/births_zylb_v1){target="_blank"}. It will be well worth consulting the documentation for this code list and the papers on which it is based if you are interested in HES birth cohorts. Next, we carry out some basic cleaning (e.g., removing implausible values), deduplicate the data and derive an academic year of birth variable. We then load the linkage spine (exactly as above) in order to link in the TPID. Finally, we save the dataset!
