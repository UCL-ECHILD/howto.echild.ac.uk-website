# Script management, coding principles and working in the SRS {#sec-mgmt}
...
The working directory in all guides assumes a simple structure containing five sub-folders: `codelists`, `csc_data`, `outputs`, `processed`, `scripts` (Figure 1). Folder `codelists` contains phenotype code lists obtained from the [ECHILD Phenotype Code List Repository](https://code.echild.ac.uk){target="_blank"} (more information in @sec-phenotype). Folder `csc_data` contains a copy of the processed CSC data (more information in @sec-csc). Folder `outputs` would contain all outputs from analyses (e.g., tables and graphs, though we never actually use this folder in these guides), `processed` will contain our processed datasets and `scripts` contains the R scripts.
...
You will also see that [00b_prelim.r](scripts/00b_prelim.r) contains the vector `year7_cohorts`, which is just `birth_cohorts + 12`. In other words, in contains the values 2015 to 2017. Again, these will refer to academic years 2014/15 to 2016/17 and are the inception years for our cohort (see Figure 1 of @sec-intro).

# Coming up...

In the next guide — @sec-cohort — we will run the first substantive script and carry out the actions necessary to create our cohort spine based on a school inception (NPD) cohort or a HES birth records cohort.
