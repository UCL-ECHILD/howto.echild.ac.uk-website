...
To add a new variable to a base R `data.frame`, we use code as in the first part of Code Snippet 3. While we could do this to a `data.table`, `data.table`’s own way is slightly different, as shown in the second part of Code Snippet 3. Note how we can perform operations on other columns within the same `data.table` easily with reference to their column name. 
...
In Code Snippet 3, we created the new column explicitly. Occasionally, however, in the guides we want to create a group of variables in a loop, where the new column is created dynamically (for example, when dealing with dual enrolment in guide 8). Study Code Snippet 4. Here we create variables that give the date that is the episode start date plus 365 days, 730 days, and so on until 3,650 days (i.e., roughly 10 years).
...
Finally, keep your scripts tidy. Take advantage of the fact that RStudio indents for you and use spaces after commas and operators just as you would when writing prose and poetry. Compare Code Snippet 6 with Code Snippet 5. All lines would run perfectly fine, but which is easier to read is clear. This is not only for your own sake six months down the road when you have to revisit old code, but also for the sake of your colleagues and future researchers who may be using the code.

*Code Snippet 6 (R, but messy - [data.table.intro.r](scripts/data.table.demo.r))*
``` r
---
epistart_vars<-names(dt_datatable )[grepl("epistart" ,names(dt_datatable ))]
