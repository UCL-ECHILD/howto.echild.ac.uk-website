...
A phenotype is any observable or measurable trait. In the present context, we are usually referring to specific conditions or groups of conditions (such as chronic conditions generally, neurodisability or congenital anomalies). Phenotypes might also describe types of presentation at hospital and broader circumstances surrounding them, including stress-related presentations and adversity-related admissions.
...
The general approach is that we convert the diagnostic and (where relevant) operation data to long format first to make it easier to work with. We then identify the codes that are in our target phenotype, deal with any special flags and then we create indicator variables that we can use later. All of this is explained in more detail in the scripts included in the Repository.

The key difference between the example scripts included in the Repository and here is that here we are dealing with two separate phenotypes. We handle this by creating a new data table called `phen_codes` that stores the phenotype codes from both code lists, identified using a variable we create called `phen_group` (Code Snippet 2). 
